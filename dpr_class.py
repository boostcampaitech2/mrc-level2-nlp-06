    ...
    def prepare_in_batch_and_hard(self,
        ...
        tokenizer=None
            ...
            if SEED == 42  and os.path.isfile(f"./p_with_neg_{self.args.per_device_train_batch_size}.pt"):
                ...
                # save hard negative samples
                torch.save(p_with_neg, DPR_PATH + f"/p_with_neg_{self.args.per_device_train_batch_size}.pt")
    ...
    def train(self,
        ...
        for e in range(self.args.num_train_epochs):
            ...
            if min_loss > epoch_loss:
                ...
                # save best encoder model in terms of minimum train loss
                
                torch.save(self.p_encoder.state_dict(), DPR_PATH + "/best_p_enc_model.pt")
                torch.save(self.q_encoder.state_dict(), DPR_PATH + "/best_q_enc_model.pt")
            ...
            if tmp_context in eval_set:
                ...
                for acc_idx, n_a in enumerate(new_acc):
                    ...
                    if best_acc[acc_idx] < n_a:
                        ...
                        # save encoder in terms of the best top k for each k in k list
                        torch.save(self.p_encoder.state_dict(), DPR_PATHf + f"/dpr_output/best_p_enc_model_top{topk_list[acc_idx]}.pt")
                        torch.save(self.q_encoder.state_dict(), DPR_PATH + f"/dpr_output/best_q_enc_model_top{topk_list[acc_idx]}.pt")   
                        
                        # save wiki passage embegging
                        torch.save(self.p_emb, DPR_PATH + f'/{tmp_context}_dense_emb/passage_embedding_top{topk_list[acc_idx]}.pt')
